...
@section('title', 'Add Chef Profile')
@section('page_title', 'Create Chef Profile')
...
<div class="w-full glass-panel rounded-2xl p-8 shadow-sm">
    ...
    <form action="{{ route('admin.chefs.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="role" id="role" value="{{ old('role') }}" placeholder="Executive Chef, GM" required
                    class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent text-sm">
            </div>
        </div>
        ...
        <div>
            <label for="image" class="block text-sm font-semibold text-body-dark mb-2">Chef Headshot</label>

...
@section('page_title', 'Edit Daily Sales')

@section('main_content')
<div class="glass-card rounded-2xl p-6 max-w-4xl mx-auto">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-lg font-bold text-white">Edit Record</h2>
    ...
    <form action="{{ route('admin.sales-targets.update', $salesTarget->id) }}" method="POST">
        @csrf
        @method('PUT')

        <div class="mb-6">
            <label class="form-label block mb-2">Date *</label>
            <input type="date" name="date" class="w-full" value="{{ old('date', $salesTarget->date) }}" required>
            ...
            <div class="bg-white/5 rounded-2xl p-5 border border-white/10">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="number" step="0.01" name="food_gross_sale" id="food_gross_sale" class="w-full" value="{{ old('food_gross_sale', $salesTarget->food_gross_sale) }}">
                    </div>
                    <div>
                        <label class="form-label block mb-2">Discounts (Pkr)</label>
                        <input type="number" step="0.01" name="food_discounts" id="food_discounts" class="w-full" value="{{ old('food_discounts', $salesTarget->food_discounts) }}">
                    </div>
                    <div>
                        <label class="form-label block mb-2">Complimentary</label>
                        <input type="number" step="0.01" name="food_complimentary" id="food_complimentary" class="w-full" value="{{ old('food_complimentary', $salesTarget->food_complimentary) }}">
                    </div>
                    <div>
                        <label class="form-label block mb-2 text-[#fce4e8]">Net sale</label>
                        <input type="number" step="0.01" name="food_net_sale" id="food_net_sale" class="w-full" value="{{ old('food_net_sale', $salesTarget->food_net_sale) }}">
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white/5 rounded-2xl p-5 border border-white/10">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="number" step="0.01" name="other_tax_income" id="other_tax_income" class="w-full" value="{{ old('other_tax_income', $salesTarget->other_tax_income) }}">
                    </div>
                    <div>
                        <label class="form-label block mb-2">Service Charges</label>
                        <input type="number" step="0.01" name="other_service_charges" id="other_service_charges" class="w-full" value="{{ old('other_service_charges', $salesTarget->other_service_charges) }}">
                    </div>
                    <div>
                        <label class="form-label block mb-2">Decor Income</label>
                        <input type="number" step="0.01" name="other_decor_income" id="other_decor_income" class="w-full" value="{{ old('other_decor_income', $salesTarget->other_decor_income) }}">
                    </div>
                    <div>
                        <label class="form-label block mb-2 text-[#fce4e8]">Total Sale</label>
                        <input type="number" step="0.01" name="other_total_sale" id="other_total_sale" class="w-full" value="{{ old('other_total_sale', $salesTarget->other_total_sale) }}">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="bg-brand text-white px-8 py-2.5 rounded-xl font-bold shadow-lg shadow-brand/20">Update Record</button>
        </div>
    </form>
</div>

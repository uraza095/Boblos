...
<div class="w-full glass-panel rounded-2xl p-8 shadow-sm">
    ...
    <form action="{{ route('admin.categories.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label for="image" class="block text-sm font-semibold text-body-dark mb-2">Category Image</label>
                <input type="file" name="image" id="image" accept="image/*"
                    class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent text-sm">
                <p class="text-xs text-gray-500 mt-1">Main category banner/image (Max size: 40MB)</p>
            </div>

            <div>
                <label for="thumbnail" class="block text-sm font-semibold text-body-dark mb-2">Thumbnail Image</label>
                <input type="file" name="thumbnail" id="thumbnail" accept="image/*"
                    class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent text-sm">
                <p class="text-xs text-gray-500 mt-1">Small icon/thumbnail image (Max size: 40MB)</p>
            </div>
        </div>
        ...
        <div class="flex items-center gap-2 py-2">
            <input type="checkbox" name="show_on_home_menu" id="show_on_home_menu" value="1" {{ old('show_on_home_menu') ? 'checked' : '' }}
                class="w-4 h-4 rounded border-gray-300 text-brand focus:ring-brand focus:ring-offset-0 bg-transparent">

    ...
    <form action="{{ route('admin.testimonials.update', $testimonial) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <textarea name="content" id="content" rows="4" required
                class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent text-sm">{{ old('content', $testimonial->content) }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
                ...
                <input type="number" name="display_order" id="display_order" value="{{ old('display_order', $testimonial->display_order) }}" min="0" required
                    class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent text-sm">
            </div>
        ...
        <div>
            <label class="block text-sm font-semibold text-body-dark mb-2">Profile Photo</label>
            @if ($testimonial->image)
                <div class="mb-3">
                    <img src="{{ asset('storage/' . $testimonial->image) }}" alt="Preview" class="w-24 h-24 object-cover rounded-full border border-gray-200">
                </div>
            @endif
